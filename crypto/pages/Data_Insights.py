...
def send_twitter_Message():
  ...
  caption = f"{st.session_state['End_Date']} {st.session_state['CurrencyPair']} Box Plot\nCrypto: ${st.session_state['Token']} {data_option} values\nToken Category: {st.session_state['TokenCategory']}\nInterval: {st.session_state['Interval']}\n{box_plot_caption}#CryptoTradingGuideBot."
...
if st.sidebar.button("View Insights"):
    ...
    box_plot.title(f"Box Plot for {data_option} from {st.session_state['Start_Date']} to {st.session_state['End_Date']}")
